...
public final class StoryPrompts {
    ...
    public static String pages(StoryTheme theme, String themeLabel, String characters, AgeGroup ag,
        ...
        return """
                ...
                매우 중요(인물 고정): 모든 페이지의 대사와 scene에는 위 "등장인물" 목록의 사람 + 단짝 친구만 등장한다.
                목록에 없는 다른 아이·친구·형제·사람을 절대 추가하지 마. scene 영문에도 이들만 묘사할 것
                (예: 주인공이 1명이면 scene에 "two children"처럼 두 명이 나오게 쓰지 말고 그 1명만 묘사).
                동물 친구는 오직 위 "단짝 친구" 하나뿐이다 — 토끼·사자·다른 부엉이 같은 새로운 동물·조력자를 절대 추가로 만들지 마라.
                사람과 동물을 절대 섞지 마라: 사람 등장인물(주인공·동생 등)은 처음부터 끝까지 "사람"이고,
                날개·부리·꼬리가 생기거나 동물(또는 단짝 친구)로 변하지 않는다.
                단짝 친구도 끝까지 정해진 한 종류다(부엉이면 부엉이) — 절대 다른 동물, 특히 "고양이"로 바뀌지 않는다.
                scene 영문에 인물을 쓸 때는 누가 누구인지 이름과 정체를 함께 밝혀 서로 헷갈리지 않게 하라
                (예: "Sangwoo, a human boy" / "the little owl friend") — 등장인물끼리 서로 뒤바뀌면 안 된다.
                (글의 언어) 서술과 대사(segments의 text)는 반드시 자연스럽고 올바른 "한국어"로만 쓴다.
                영어 단어·문장, 로마자 표기를 쓰지 마라(영문은 오직 그림용 "scene" 필드에만). 의미 없는 단어·가짜 낱말·깨진 글자 금지.
