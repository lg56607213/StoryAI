...
public class GeminiImageGenerator implements ImageGenerator {
    ...
    @Override
    public byte[] everydaySheet(List<byte[]> photos, String name) {
        String prompt = "You are given several photos of the SAME real young child"
                ...
                + "pattern - do NOT put the child in any costume or dress. " + STYLE
    ...
    @Override
    public byte[] costumeSheet(byte[] everydaySheet, CharacterRole role, String name) {
        String outfit = role == CharacterRole.MAIN
                ? "a soft pastel pink princess dress with a small tiara and a floral headband"
        ...
        String prompt = "This reference image is a young child storybook character. Keep the EXACT same face, "
                + "likeness, cheeks, eyes, smile and hairstyle identical, but change the clothing to " + outfit + ". "
                + STYLE + " Full body, standing, plain warm cream background, centered. No text, no watermark.";
